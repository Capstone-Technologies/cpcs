...
include "config.php";
session_start();
$get_id=mysqli_fetch_array(mysqli_query($con,"select max(cand_id) from cand_data"));
$candidate_id=$get_id[0];
$query=mysqli_query($con,"select fname,mname,lname,designation,company,cur_city from cand_data where cand_id =".$candidate_id."");
$full_name='lname'.','.'fname'.''.'mname';
$can_desig='designation';
$can_company='company';
$cur_city='cur_city';

$display_result=$con($query);
$row_count=$result($query);
?>
        	...
        	<div class="rslt_disp">
            	<table>
                	<?php 	for($i=0;$i<=$row_count;$i++):
							$display_result=$result->fetch_assoc();
					?>
					<tbody>
                        <td><div class="add_can_name"><label class="add_can_name_lbl">Name</label></div></td>
                        <td><div class="update_name_txt1"><label id="full_name"><?php echo $display_result['fullname']  ?>Tanwar, Yogendra Singh</label></div></td>
						<td><div class="add_can_name"><label class="add_can_name_lbl">Current Designation</label></div></td>
                        <td><span class="update_name_txt1"><label id="can_desig"><?php echo $display_result['designation']  ?>Manager</label></span></td>
                        <td><div class="add_can_name"><label class="add_can_name_lbl">Organization</label></div></td>
                        <td><span class="update_name_txt1"><label id="can_company"><?php echo $display_result['company']  ?>Capstone Consultant</label></span></td>
						<td><div class="add_can_name"><label class="add_can_name_lbl">Location</label></div></td>
                        <td><span class="update_name_txt1"><label id="cur_city"><?php echo $display_result['cur_city']  ?>Gurgaon</label></span></td>
                    </tbody>
                    <?php endfor; ?>
                    <tbody>
